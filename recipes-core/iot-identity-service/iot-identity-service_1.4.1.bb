...
# Uncreate /usr/lib/rust for shared libraries
export rustlibdir = "${libdir}"
...
SRC_URI += " \
           ...
           git://github.com/Azure/iot-identity-service.git;protocol=https;nobranch=1 \
           file://0001-Updated-openssl-errors-version-to-0.2.patch \
           file://0002-Updated-engine.c-for-openssl3.patch \
           file://0003-Panic-strategy-changed-from-abort-to-unwind.patch \
"
...
DEPENDS += " openssl cbindgen-native bindgen-native libtss2-dev clang-native"
PROVIDES += " iot-identity-service"
RPROVIDES:${PN} += " iot-identity-service"
...
do_compile () {
        ...
        cbindgen --config key/aziot-keys/cbindgen.toml --crate aziot-keys --output /dev/stdout | \
        ...
        if ! diff -q key/aziot-keys/aziot-keys.h "$$aziot_keys_h_new"; then \
                ...
                rm -f key/aziot-keyd/src/keys.generated.rs; \
        fi ; \
        popd
...
do_install () {
        # Binaries
        install -m 755 -D ${B}/target/${CARGO_TARGET_SUBDIR}/aziotd ${D}${libexecdir}/aziot-identity-service/aziotd
        ln -s ${libexecdir}/aziot-identity-service/aziotd ${D}${libexecdir}/aziot-identity-service/aziot-certd
        ln -s ${libexecdir}/aziot-identity-service/aziotd ${D}${libexecdir}/aziot-identity-service/aziot-identityd
        ln -s ${libexecdir}/aziot-identity-service/aziotd ${D}${libexecdir}/aziot-identity-service/aziot-keyd
        ln -s ${libexecdir}/aziot-identity-service/aziotd ${D}${libexecdir}/aziot-identity-service/aziot-tpmd

        install -m 755 -D ${B}/target/${CARGO_TARGET_SUBDIR}/aziotctl ${D}${bindir}/aziotctl

        # libaziot-keys
        install -m 755 -D ${B}/target/${CARGO_TARGET_SUBDIR}/libaziot_keys.so ${D}${rustlibdir}/libaziot_keys.so

        # Default configs and config directories
        install -m 644 -D ${WORKDIR}/git/cert/aziot-certd/config/unix/default.toml ${D}${sysconfdir}/aziot/certd/config.toml.default
        install -d -m 0700 ${D}${sysconfdir}/aziot/certd/config.d

        install -m 644 -D ${WORKDIR}/git/identity/aziot-identityd/config/unix/default.toml ${D}${sysconfdir}/aziot/identityd/config.toml.default
        install -d -m 0700 ${D}${sysconfdir}/aziot/identityd/config.d

        install -m 644 -D ${WORKDIR}/git/key/aziot-keyd/config/unix/default.toml ${D}${sysconfdir}/aziot/keyd/config.toml.default
        install -d -m 0700 ${D}${sysconfdir}/aziot/keyd/config.d

        install -m 644 -D ${WORKDIR}/git/tpm/aziot-tpmd/config/unix/default.toml ${D}${sysconfdir}/aziot/tpmd/config.toml.default
        install -d -m 0700 ${D}${sysconfdir}/aziot/tpmd/config.d

        install -m 644 -D ${WORKDIR}/git/aziotctl/config/unix/template.toml ${D}${sysconfdir}/aziot/config.toml.template

        # Systemd services and sockets
        for i in cert identity key tpm; do \
                OUTPUT_SERVICE="${D}${systemd_unitdir}/system/aziot-${i}d.service"; \
                install -m 644 -D "${WORKDIR}/git/${i}/aziot-${i}d/aziot-${i}d.socket" "${D}${systemd_unitdir}/system/aziot-${i}d.socket"; \
                <"${WORKDIR}/git/${i}/aziot-${i}d/aziot-${i}d.service.in" sed \
                        -e 's|@private-libs@|${libexecdir}/aziot-identity-service|' \
                        -e 's|@libexecdir@|${libexecdir}|' \
                        >"${OUTPUT_SERVICE}"; \
                chmod 0644 "${OUTPUT_SERVICE}"; \
        done
        # TODO? : Need copy libaziot_key_openssl_engine_shared.so to /usr/lib/ossl-modules/aziot_keys.so
}

FILES:${PN} += " \
    ${systemd_unitdir}/system/* \
"
# Disabled moving /usr/lib/libaziot_keys.so to dev package.
...
inherit useradd
USERADD_PACKAGES = "${PN}"
USERADD_PARAM:${PN} = "-r -d ${localstatedir}/lib/aziot/identityd -m -k /dev/null -s /bin/false aziotid; "
USERADD_PARAM:${PN} += "-r -d ${localstatedir}/lib/aziot/certd -m -k /dev/null -s /bin/false aziotcs; "
USERADD_PARAM:${PN} += "-r -d ${localstatedir}/lib/aziot/keyd -m -k /dev/null -s /bin/false aziotks; "
USERADD_PARAM:${PN} += "-r -d ${localstatedir}/lib/aziot/tpmd -m -k /dev/null -s /bin/false aziottpm; "
